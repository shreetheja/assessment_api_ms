const { Api500Error, Api401Error, Api200Success, Api400Error } = require('../../handlers/apiErrors');
    ...
    static async login(req, res) {
        ...
        try {
            ...
            if(assessmentMetaData.error ){
                this.sendApi500(res);
                return;
            }else if (assessmentMetaData.rows.length <= 0) {
                const responseObj =new Api400Error(
                    'Assessment is either not present or not started yet ..',
                    'Assessment is either not present or not started yet ..'
                );
